...
The predefined flavors are packages served from OBS:
 ...
 - https://build.opensuse.org/package/show/systemsmanagement:saltstack:products/salt[products]
 - https://build.opensuse.org/package/show/systemsmanagement:saltstack:products:testing/salt[products:testing]
 - https://build.opensuse.org/package/show/systemsmanagement:saltstack:products:next/salt[products:next]
...
cd salt-toaster
virtualenv sandbox
virtualend --relocatable sandbox
...
We can do so by running one of these self explanatory commands
[source,bash]
----
make saltstack.integration
make saltstack.unit
make suse.tests
----
...
At the moment, the defaults are `sles12sp1` and https://build.opensuse.org/package/show/systemsmanagement:saltstack:products/salt[`saltstack:products:salt`]

The first thing that this command does is to pull the right container image from the registry(hardcoded).

With the default values the image would be: `registry.mgr.suse.de/toaster-sles12sp1-products`
